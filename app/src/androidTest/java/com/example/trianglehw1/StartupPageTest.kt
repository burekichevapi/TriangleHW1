...
class StartupPageTest {
    ...
    @Test
    fun StartupPageShowsInstructions() {
        val activityScenario = ActivityScenario.launch(MainActivity::class.java)
        //Setup
        val textView = onView(withId(R.id.triangle))

        //Verify
        textView.check(matches(withSubstring("Purpose:")))
        textView.check(matches(withSubstring("The Triangle App will display the different types of triangles that can be created depending on the inputted triangle side lengths:")))
        ...
        textView.check(matches(withSubstring("One line of symmetry")))
        textView.check(matches(withSubstring("Scalene")))
        textView.check(matches(withSubstring("Two sides of equal lengths")))
        textView.check(matches(withSubstring("Two equal angles")))
        textView.check(matches(withSubstring("One line of symmetry")))
